"""
NAPA Auto Parts connector.

Scrapes NAPA search results from server-rendered HTML.
...
class NAPAConnector(BaseConnector):
    """NAPA Auto Parts scraper with link fallback."""

    def __init__(self):
        super().__init__("napa")

    async def search(self, query: str, **kwargs) -> dict[str, Any]:
        """Search NAPA. Scrapes when enabled, falls back to links."""
        ...
        try:
            ...
            if results["market_listings"]:
                ...
                return results
            logger.info("NAPA scrape returned 0 listings")
        except Exception as e:
            logger.warning(f"NAPA scrape failed: {e}")

        return self._generate_links(query, kwargs)

    async def _scrape(self, query: str, **kwargs) -> dict[str, Any]:
        """Fetch and parse NAPA search results."""
        ...
        products = soup.select(".product-card, [class*='product-card'], [class*='ProductCard'], .search-result-product")
        ...
        for product in products:
            ...
            if link_tag:
                ...
                if href.startswith("/"):
                    product_url = f"https://www.napaonline.com{href}"
    ...
    def _generate_links(self, query: str, kwargs: dict = None) -> dict[str, Any]:
        ...
        for pn in (kwargs or {}).get("part_numbers", []):
            ...
            links.append(
                ExternalLink(
                    label=f"NAPA: {pn}",
                    url=f"https://www.napaonline.com/en/search?q={encoded_pn}",
                    source="napa",
                    category="new_parts",
                )
            )
