...
def normalize_part_number(part_num: str) -> str:
    """
    Normalize a single part number for display/query.
    - Convert to uppercase, remove spaces, preserve dashes and dots.
    """
    if not part_num:
        return ""
...
def part_number_value_norm(value: str, strip_hyphens: bool = True) -> str:
    """
    Normalize part number value for canonical storage and matching (value_norm).
    - Uppercase, strip spaces.
    - If strip_hyphens True (default), remove hyphens so 123-456 and 123456 match.
    """
    if not value:
        return ""
    s = value.upper().strip().replace(" ", "")
    if strip_hyphens:
        s = s.replace("-", "")
    return s
